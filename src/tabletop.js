...
var Tabletop = function(options) {
  ...
  var visibility = this.authkey ? 'private' : 'public';
  var format = (inNodeJS || supportsCORS) ? 'json' : 'json-in-script';
  this.base_json_path = `/feeds/worksheets/${this.key}/${visibility}/full?alt=${format}`;
...
Tabletop.prototype = {
  ...
  loadSheets: function(data) {
    ...
    for (i = 0, ilen = data.feed.entry.length; i < ilen ; i++) {
      ...
      if (this.isWanted(data.feed.entry[i].content.$t)) {
        ...
        json_path += ((inNodeJS || supportsCORS) ? 'json' : 'json-in-script');
  ...
  */
  ...
  sheetReady: function(model) {
    ...
    if(ttIndexOf(this.model_names, model.name) === -1) this.model_names.push(model.name);
    this.sheetsToLoad--;
  ...
  */
  loadSheet: function(data) {
    ...
    var model = new Tabletop.Model({
      ...
      onReady: function() { self.sheetReady(this); }
    });
  },
